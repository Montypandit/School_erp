...
const StatCard = ({ title, value, percentage, icon, color }) => {
  return (
    ...
    >
      ...
      <h3 style={{ margin: '10px 0 5px' }}>{title}</h3>
      <p style={{ margin: 0, fontWeight: 'bold', fontSize: '18px' }}>{value}</p>
      {percentage !== undefined && <p style={{ color: color }}>{Math.round(percentage)}%</p>}
    </div>
  );
};
...
const AdminHome = () => {
  ...
  // Helper to get actual JWT token string from sessionStorage
  const getToken = () => {
  ...
  useEffect(() => {
    ...
    // Fetch present and absent students today
    const fetchTotalPresntAbsentStudents = async () => {
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error('Failed to fetch present students data');
        }
    ...
    // Fetch total teachers count and total employees
    const fetchTotalTeachers = async () => {
      try {
        const res = await fetch(
  ...
  const presentStudentPercentage = totalStudents > 0 ? (presentStudents / totalStudents) * 100 : 0;
  const absentStudentPercentage = totalStudents > 0 ? (absentStudents / totalStudents) * 100 : 0;
  ...
  return (
    ...
    >
      ...
      >
        <StatCard title="Total Students (All Time)" value={allTimeTotalstudents} icon="👥" color="#4CAF50" />
        <StatCard title="Current Year Students" value={totalStudents} icon="👥" color="#4CAF50" />
        <StatCard title="Total Employees" value={totalEmployees} percentage={100} icon="👨‍🏫" color="#4CAF50" />
        <StatCard title="Total Teachers" value={totalTeachers} percentage={100} icon="👨‍🏫" color="#4CAF50" />
        <StatCard
          title="Present Students"
          value={`${presentStudents}/${totalStudents}`}
          percentage={presentStudentPercentage}
        ...
        <StatCard
          title="Teachers Present"
          value={`${totalTeachers}/${totalTeachers}`}
          percentage={presentTeacherPercentage}
          icon="👨‍🏫"
          color="#9C27B0"
        />
        <StatCard title="Absent Teachers" value={0} percentage={0} icon="👨‍🏫" color="#4CAF50" />
      </div>
