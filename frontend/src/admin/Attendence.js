...
export default function AttendanceDashboard() {
    ...
    const fetchData = async () => {
      try {
        const [teacherRes, studentRes, classRes] = await Promise.all([
          fetch("http://localhost:5000/api/attendance/teachers", {
            headers: { Authorization: `Bearer ${token}` },
          }),
          fetch("http://localhost:5000/api/attendance/students", {
            headers: { Authorization: `Bearer ${token}` },
          }),
  ...
  const filteredTeachers = teacherAttendance.filter((t) => {
              ...
              <tbody>
                {filteredTeachers.map((t) => (
                  <tr key={t.id} className="border-t">
                    <td className="p-2 border font-medium">{t.id}</td>    
                    <td className="p-2 border font-medium">{t.name}</td>
                    <td className="p-2 border font-medium">{t.email}</td>
                    <td className="p-2 border">{getStatusBadge(t.status)}</td>
              ...
              <tbody>
                {filteredStudents.map((s) => (
                  <tr key={s.id} className="border-t">
                    <td className="p-2 border font-medium">{s.rollNo}</td>
                    <td className="p-2 border">{s.name}</td>
                    <td className="p-2 border">{s.email}</td>
                    <td className="p-2 border">{classes.find((c) => c.id === s.classId)?.name || ""}</td>
