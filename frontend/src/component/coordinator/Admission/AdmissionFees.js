...
const FeesGeneration = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        try {
          response = await fetch(`http://localhost:5000/api/final/admission/get/student/${admissionId}`, {
            method: 'GET',
            headers: {
        ...
        if (!data) {
          try {
            response = await fetch(`http://localhost:5000/api/admission/get/${admissionId}`, {
              method: 'GET',
              headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`,
            ...
            console.log('Second endpoint also failed');
          }
        }
  ...
  const generateFeeReceiptPDF = (data) => {
    ...
    doc.text(`₹ ${subtotal.toFixed(2)}`, 155, y + 3);
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  fullWidth
                  type="number"
                  label="Total Admission Fees (If Different)"
